...
from transformers import (WEIGHTS_NAME, AdamW, get_linear_schedule_with_warmup,
                          GPT2Model, GPT2Tokenizer, GPT2Model)
    ...
    for idx in range(args.epochs): 
        ...
        for step, batch in enumerate(bar):
            ...
            if args.use_focal_loss:
                ...
                loss, _ = model(input_ids=input_ids, logit_adjustment=logit_adjustment, labels=labels)
            if args.n_gpu > 1:
                loss = loss.mean()
...
def evaluate(args, model, tokenizer, eval_dataset, eval_when_training=False):
    ...
    for batch in eval_dataloader:
        (input_ids, labels)=[x.to(args.device) for x in batch]
        with torch.no_grad():
            prob = model(input_ids=input_ids)      
...
def main():
    ...
    parser.add_argument("--use_logit_adjustment", action='store_true',
                        help="")
    ...
    decoder = GPT2Model.from_pretrained(args.model_name_or_path, num_labels=len(cwe_label_map))    
    decoder.config.pad_token_id = tokenizer.pad_token_id
    model = Model(decoder, decoder.config, tokenizer, args, len(cwe_label_map))
