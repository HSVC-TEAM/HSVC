...
from transformers import (get_linear_schedule_with_warmup, RobertaTokenizer, RobertaModel, RobertaConfig)
...
def train(args, train_dataset, model, teacher_model, tokenizer, eval_dataset, cwe_label_map):
    ...
    for idx in range(args.epochs): 
        ...
        for step, batch in enumerate(bar):
            ...
            model.train()
            teacher_logits = teacher_model(source_ids=source_ids, position_idx=position_idx, attn_mask=attn_mask, group=group)
...
def main():
    ...
    roberta_1 = RobertaModel.from_pretrained(args.model_name_or_path)
    roberta_2 = RobertaModel.from_pretrained(args.model_name_or_path) 
    roberta_3 = RobertaModel.from_pretrained(args.model_name_or_path)
    roberta_4 = RobertaModel.from_pretrained(args.model_name_or_path)
    roberta_5 = RobertaModel.from_pretrained(args.model_name_or_path)

    g1_model = TextCNN(roberta=roberta_1, 
                    dim_channel=100, 
                    kernel_wins=[3,4,5], 
                    dropout_rate=0.1, 
                    num_class=len(cwe_label_map), 
                    args=args)
    g2_model = TextCNN(roberta=roberta_2, 
                    dim_channel=100, 
                    kernel_wins=[3,4,5], 
                    dropout_rate=0.1, 
                    num_class=len(cwe_label_map), 
                    args=args)
    g3_model = TextCNN(roberta=roberta_3, 
                    dim_channel=100, 
                    kernel_wins=[3,4,5], 
                    dropout_rate=0.1, 
                    num_class=len(cwe_label_map), 
                    args=args)
    g4_model = TextCNN(roberta=roberta_4, 
                    dim_channel=100, 
                    kernel_wins=[3,4,5], 
                    dropout_rate=0.1, 
                    num_class=len(cwe_label_map), 
                    args=args)
    g5_model = TextCNN(roberta=roberta_5, 
                    dim_channel=100, 
                    kernel_wins=[3,4,5], 
                    dropout_rate=0.1, 
                    num_class=len(cwe_label_map), 
    ...
    checkpoint_prefix = 'checkpoint-best-acc/t1.bin'
    output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))  
    g1_model.load_state_dict(torch.load(output_dir, map_location=args.device))  

    checkpoint_prefix = 'checkpoint-best-acc/t2.bin'
    output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix)) 
    g2_model.load_state_dict(torch.load(output_dir, map_location=args.device))

    checkpoint_prefix = 'checkpoint-best-acc/t3.bin'
    output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix)) 
    g3_model.load_state_dict(torch.load(output_dir, map_location=args.device))

    checkpoint_prefix = 'checkpoint-best-acc/t4.bin'
    output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix)) 
    g4_model.load_state_dict(torch.load(output_dir, map_location=args.device))

    checkpoint_prefix = 'checkpoint-best-acc/t5.bin'
    output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix)) 
    g5_model.load_state_dict(torch.load(output_dir, map_location=args.device))
    ...
    teacher_model = CombinedTeacher(g1_model, g2_model, g3_model, g4_model, g5_model, config, tokenizer, args)
    teacher_model.to(args.device)

    gcb = RobertaModel.from_pretrained(args.model_name_or_path)
    gcb.resize_token_embeddings(len(tokenizer))
    model = StudentGraphCodeBERT(encoder=gcb, tokenizer=tokenizer, config=gcb.config, num_labels=len(cwe_label_map),args=args)
